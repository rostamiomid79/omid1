...
class Transaction(models.Model):
    ...
    transaction_type = models.PossitiveSmallIntegerField(choices=TRANSACTION_TYPE_CHOICES, default=CHARGE)
    amount = models.BigIntegerField()
    created_time = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.user} - {self.get_trnsaction_type_display} - {self.amount}"

    @classmethod
    def get_report(cls):
        positive_transactions = Sum('transactions__amount', filter=Q(transactions__transaction_type__in=[1,3]))
        negative_transactions = Sum('transactions__amount', filter=Q(positive__transaction_type_in=[2,4]))
        users = User.objects.all().annotate(
            transactions_count= Count('transaction__id'),
    ...
    def user_balance(cls, user):
        ...
        negative_tranasctions = Sum('amount', filter=Q(transaction_type__in=[2, 4]))
        user_balance = user.transactions.all().aggregate(
            balance=Coalesce(positive_transactions, 0) - Coalesce(negative_tranasctions, 0)
        )
...
class UserBalance(models.Model):
    user = models.ForeignKey(User, related_name='balance_record')
...
class TransferTransaction(models.Model):
    sender_transaction = models.ForeignKey(Transaction, related_name='sent_transfers' on_delete=models.RESTRICTE)
    receiver_transaction = models.ForeignKey(Transaction, related_name='received_transfers', on_delete=models.RESTRICTE)
    ...
    def transfer(cls, sender, receiver, amount):
        if Transaction.user_balance(sender) < amount:
            return "Transaction Not Allowed, insufficient balances"
        with transaction.atomic():
            ...
            receiver_transaction = Transaction.objects.create(
                user=user, amount=amount, transaction_type=Transaction.TRANSFER_RECEIVED
            )
...
class UserScore(models.Model):
    ...
    score = models.PossitiveSmallIntegerField()
